...
import in.ineuron.managelibrary.*;
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(request.getRequestURI().endsWith("updateform")) {
			...
			if(msg.equalsIgnoreCase("success")) {
				out.print("<h1> Updation successfull</h1>");
			}
			else {
		...
		if(request.getRequestURI().endsWith("searchform")) {
			...
			out.print("  <thead>\r\n"
				...
				out.print("<h1> Student not found with the id "+request.getParameter("Student id")+"</h1>");
			}
		}
		if(request.getRequestURI().endsWith("deleteform")) {
		String msg=	S.deletestudent(Integer.parseInt(request.getParameter("Student id")));
			if(msg.equalsIgnoreCase("success")) {
				out.print("<h1> Student deleted</h1>");
			}
			else {
				out.print("<h1> Student deletion failed</h1>");
			}
		}
		out.println("<form>\r\n"
