		...
		if(request.getRequestURI().endsWith("student")) {
			...
			if(msg.equalsIgnoreCase("success")) {
				try {
					...
					pst=con.prepareStatement("select sid from student where name=?,branch=?,password=?,email=?");
				...
				out.print("<h1 style=color:green> REGISTRATION SUCCESSFUL YOUR ID IS "+id+"</h1>");
			}
			else {
